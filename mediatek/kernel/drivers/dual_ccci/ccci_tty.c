...
static ssize_t ccci_tty_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
	...
	if( (read + data_be_read) >= length ) {
		...
		if (copy_to_user(u_buf,&rx_buffer[read],part)) {
			CCCI_MSG_INF(md_id, "tty", "read: copy_to_user fail\n");
			ret= -EFAULT;
			goto out;
		}
		// Copy second part
...
static ssize_t ccci_tty_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if( (write+data_be_write) >= length) {
		...
		// 1st write
		part = length - write;
		if (copy_from_user(&tx_buffer[write],buf,part))
		{
			CCCI_MSG_INF(md_id, "tty", "write: copy from user fail \n");
			ret=-EFAULT;
			goto out;
		}
		// 2nd write
		if (copy_from_user(tx_buffer,&buf[part],data_be_write - part))
		{
			CCCI_MSG_INF(md_id, "tty", "write: copy from user fail \n");
			ret=-EFAULT;
			goto out;
		...
		// Write once is OK
		if (copy_from_user(&tx_buffer[write],buf,data_be_write))
		{
			CCCI_MSG_INF(md_id, "tty", "write: copy from user fail \n");
			ret=-EFAULT;
			goto out;
