...
int mali_pm_restore_noirq(struct device *device)
{
    mt_irq_set_sens(MT_MFG_IRQ_GP_ID, MT_LEVEL_SENSITIVE);
    mt_irq_set_polarity(MT_MFG_IRQ_GP_ID, MT_POLARITY_LOW);

    mt_irq_set_sens(MT_MFG_IRQ_GPMMU_ID, MT_LEVEL_SENSITIVE);
    mt_irq_set_polarity(MT_MFG_IRQ_GPMMU_ID, MT_POLARITY_LOW);

    mt_irq_set_sens(MT_MFG_IRQ_PP0_ID, MT_LEVEL_SENSITIVE);
    mt_irq_set_polarity(MT_MFG_IRQ_PP0_ID, MT_POLARITY_LOW);

    mt_irq_set_sens(MT_MFG_IRQ_PPMMU0_ID, MT_LEVEL_SENSITIVE);
    mt_irq_set_polarity(MT_MFG_IRQ_PPMMU0_ID, MT_POLARITY_LOW);

    mt_irq_set_sens(MT_MFG_IRQ_PP1_ID, MT_LEVEL_SENSITIVE);
    mt_irq_set_polarity(MT_MFG_IRQ_PP1_ID, MT_POLARITY_LOW);

    mt_irq_set_sens(MT_MFG_IRQ_PPMMU1_ID, MT_LEVEL_SENSITIVE);
    mt_irq_set_polarity(MT_MFG_IRQ_PPMMU1_ID, MT_POLARITY_LOW);
...
#ifdef CONFIG_MTK_HIBERNATION
    register_swsusp_restore_noirq_func(ID_M_MALI, mali_pm_restore_noirq, NULL);
#endif
...
#ifdef CONFIG_MTK_HIBERNATION
    unregister_swsusp_restore_noirq_func(ID_M_MALI);
#endif
}
