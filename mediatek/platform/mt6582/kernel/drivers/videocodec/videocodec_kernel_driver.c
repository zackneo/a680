    ...
    switch(cmd) {
        ...
        case VCODEC_LOCKHW:
                     ...
                     rHWLock.eDriverType == VAL_DRIVER_TYPE_JPEG_ENC)
            {
                ...
                while (bLockedHW == VAL_FALSE)
                {
                    ...
                    {
                        FirstUseEncHW = 0;
                        if (rHWLock.u4TimeoutMs == 0)//for JPEG ENC
                        {
                    ...
                    {
                        MFV_LOGE("Lock ok grVcodecEncHWLock.pvHandle = 0x%x, va:%x, type:%d", grVcodecEncHWLock.pvHandle, (unsigned int)rHWLock.pvHandle, rHWLock.eDriverType);//TODO for debug remove someday
                    }
                    mutex_unlock(&VencHWLock);
                }
        ...
        case VCODEC_WAITISR:
            ...
            else if (val_isr.eDriverType == VAL_DRIVER_TYPE_H264_ENC)
            {
                ...
                //trace_printk("[VR power] H.264 VENC ENGINE ON\n");
                eValRet = eVideoWaitEvent(&EncIsrEvent, sizeof(VAL_EVENT_T));
                if(VAL_RESULT_INVALID_ISR == eValRet)
                {
                    return -2;
                }
                //trace_printk("[VR power] H.264 VENC ENGINE OFF\n");
