...
//    unsigned int status;
    ...
    {   
            case DISP_IOCTL_RESOURCE_REQUIRE:
                ...
                {
                    DISP_ERR("DISP_IOCTL_RESOURCE_REQUIRE : Copy to user error\n");
                    return -EFAULT;
                }
    
      
                printk(KERN_DEBUG "=========DISP_IOCTL_RESOURCE_REQUIRE taskID:%d==========\n",taskID);
        ...
        case DISP_IOCTL_EXEC_COMMAND:
           ...
           //spin_lock_irqsave(&gCmdqMgrLock,flags); 
            ...
            {
                DISP_ERR("disp driver : Copy from user error\n");
           //     spin_unlock_irqrestore(&gCmdqMgrLock,flags);              
                return -EFAULT;
            }
            //printk(KERN_DEBUG "==========DISP_IOCTL_EXEC_COMMAND Task: %d start at SEC: %d MS: %d===========\n",cParams.taskID, (unsigned int)tv.tv_sec, (unsigned int)tv.tv_usec);
            ...
            printk(KERN_DEBUG "CMDQ task %x buffer VA: 0x%lx MVA: 0x%lx \n", pCmdqAddr->Owner, pCmdqAddr->VA, pCmdqAddr->MVA);
...
static int disp_suspend(struct platform_device *pdev, pm_message_t mesg)
{
    ...
    //Only Thread 0 available in 6582
    if(true == checkMdpEngineStatus(0x03FF))
    {
        printk("MDP still runnig but force turn-off by suspend\n");
        return -EFAULT;
    }
