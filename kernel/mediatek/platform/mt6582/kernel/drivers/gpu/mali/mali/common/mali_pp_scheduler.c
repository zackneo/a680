...
static void sync_callback(struct sync_fence *fence, struct sync_fence_waiter *waiter)
{
	...
	MTKPP_LOG(MTKPP_ID_SYNC, "sigl fence FD: %d", job->uargs.fence);
...
#warning PP jobs will not be executed
	...
	if (0 < job->uargs.fence)
	{
		...
		MTKPP_LOG(MTKPP_ID_SYNC, "wait fence FD: %d", pre_fence_fd);
		...
		if (1 == err)
		{
			MTKPP_LOG(MTKPP_ID_SYNC, "sigled fence FD: %d", pre_fence_fd);
		...
		{
			MTKPP_LOG(MTKPP_ID_SYNC, "erro fence FD: %d", pre_fence_fd);
